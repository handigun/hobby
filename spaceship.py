...
class Sprite:
    ...
    def collide(self, other_sprite):
        other_sprite_radius = other_sprite.radius
        other_sprite_pos = other_sprite.pos
        distance = dist(self.pos,other_sprite.pos)
        if distance < (other_sprite_radius+self.radius):
            return True
        return False
...
def draw(canvas):
    global time, started
    global lives, score, rock_group, missile_group
    # animiate background
    time += 1
    wtime = (time / 4) % WIDTH
...
def group_collide(group, other_sprite):
    ...
    for sprite in group:
        if(sprite.collide(other_sprite)):
            a_explosion = Sprite(sprite.pos, [0,0], 0, 0, explosion_image, explosion_info)
